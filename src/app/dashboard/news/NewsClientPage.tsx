...
export default function NewsClientPage({ initialAnnouncements }: NewsClientPageProps) {
  ...
  // STRICT RULE: No mock data! Only real DB data or EmptyState.
    ...
    <div className="max-w-7xl mx-auto space-y-12 pb-24">
        ...
        <div className="flex gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-72">
            ...
            <input 
              ...
              className="w-full pl-12 pr-6 py-4 rounded-2xl bg-white border border-outline-variant/10 cursor-not-allowed opacity-50 text-sm font-medium" 
            />
          </div>
          <button disabled className="p-4 rounded-2xl bg-white border border-outline-variant/10 text-primary cursor-not-allowed opacity-50">
      ...
      {announcements.length === 0 ? (
        <EmptyState
          ...
          description="Votre fil d'actualités est vide. Les communications officielles de l'université apparaîtront ici."
        />
      ) : (
        <>
          {/* Featured Article */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-10">
              ...
              <div className="space-y-8">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-2">
                      <div className="space-y-3">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/40">{news.timeAgo || new Date(news.created_at).toLocaleDateString()}</span>
                        </div>
                        <h4 className="text-2xl font-black font-headline text-slate-900 group-hover:text-primary transition-colors tracking-tighter leading-[1.2]">
                          {news.title}
                        </h4>
                        <p className="text-slate-600 text-base line-clamp-2 leading-relaxed font-medium">
                      ...
                      <div className="flex items-center gap-6 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button className="p-2 rounded-xl hover:bg-slate-100 transition-colors text-slate-400 hover:text-primary">
                ...
                {filteredAnnouncements.length <= 1 && (
                  <p className="text-center text-sm font-medium text-slate-400 py-10">Aucun autre article pour cette catégorie.</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-10">
              <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100 shadow-inner">
                <h3 className="text-2xl font-black font-headline text-slate-900 mb-8 flex items-center gap-4 tracking-tight">
                  <Bell className="h-7 w-7 text-amber-500" />
                  Alertes Flash
                </h3>
                <div className="space-y-6">
                  {/* Empty state for flash alerts since we have no DB table for them yet */}
                  <div className="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 text-center">
                    <p className="text-sm font-medium text-slate-500">Aucune alerte flash en cours.</p>
                  </div>
                </div>
              </div>

              <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-10 rounded-[3rem] text-white flex flex-col justify-between h-[400px] shadow-2xl relative overflow-hidden group">
                <Newspaper className="h-16 w-16 opacity-20 transition-transform group-hover:scale-110 duration-700" />
                <div className="relative z-10">
                  <h4 className="text-2xl font-black font-headline mb-4 tracking-tighter leading-tight">Newsletter Campus</h4>
                  <p className="text-sm opacity-70 leading-relaxed font-medium mb-8">Ce service de newsletter par email ouvrira très prochainement.</p>
                  <div className="relative opacity-50">
                    <input disabled type="email" placeholder="Indisponible" className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-6 pr-12 text-sm placeholder:text-white/40 cursor-not-allowed outline-none" />
                    <button disabled className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-xl bg-white/20 text-white cursor-not-allowed">
                      <ArrowRight className="h-5 w-5" />
                    </button>
                  </div>
                </div>
