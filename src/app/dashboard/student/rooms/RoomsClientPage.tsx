    ...
    <div className="max-w-[1400px] mx-auto animate-in fade-in duration-700">
      ...
      <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-full bg-surface-container-lowest text-on-surface font-bold border border-outline-variant/20 hover:bg-surface-container-low transition-all shadow-sm">
            <Filter className="h-4 w-4" />
            <span>Filtres avancés</span>
          </button>
          <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-full bg-gradient-to-br from-primary to-primary-container text-white font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all active:scale-95">
      ...
      <footer className="mt-20 pt-10 border-t border-outline-variant/10 flex flex-col sm:flex-row justify-between items-center gap-4 text-on-surface-variant text-xs font-bold uppercase tracking-widest">
        <div className="flex items-center gap-8">
          ...
          <a className="hover:text-primary transition-colors" href="#">Règlement</a>
          <a className="hover:text-primary transition-colors" href="#">Signaler</a>
        </div>
        <p className="opacity-60">© 2024 CampusConnect - Direction du Numérique</p>
      </footer>
    </div>
  )
}
